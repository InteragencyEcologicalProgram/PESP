...
df_smscg <- extract_program_taxa(df_smscg)
...
df_smscg <- df_smscg %>%
  mutate(
    Notes = case_when(
      tolower(QualityCheck) == 'fragmented' ~ 'FragmentedDiatoms',
      TRUE ~ 'Unknown'
    ),
    QualityCheck = case_when(
      tolower(QualityCheck) == 'good' ~ 'NoCode',
      tolower(QualityCheck) == 'fragmented' ~ 'NoCode',
      tolower(QualityCheck) == 'degraded' ~ 'Degraded',
      TRUE ~ QualityCheck
    ),
    Debris = case_when(
      tolower(Debris) == 'high' ~ 'High',
      tolower(Debris) == 'moderate' ~ 'Moderate',
      tolower(Debris) == 'low' ~ 'Low',
      is.na(Debris) ~ 'None',
      TRUE ~ Debris
    )
  )
...
df_smscg <- df_smscg %>%
  mutate(
    QualityCheck = case_when(
      Cells_per_mL < Units_per_mL & (QualityCheck == 'Good' | QualityCheck == 'NoCode') ~ 'UnitsExceedCells',
      Cells_per_mL < Units_per_mL & !QualityCheck == 'Good' & QualityCheck != 'NoCode' ~ paste(QualityCheck, 'UnitsExceedCells', sep = ' '),
      TRUE ~ QualityCheck
    )
  )
...
```{r}
df_smscg <- combine_taxa(df_smscg, key_cols = c('Date', 'Time', 'Station', 'SampleDepth'))
...
```{r}
df_smscg <- remove_nodata(df_smscg)
no_data <- attr(df_smscg, 'log')$nodata
```

## Check for outliers
```{r}
# Cells per mL
df_smscg <- flag_outliers(df_smscg, Cells_per_mL, cutoff = 5)
```

```{r}
# Units per mL
df_smscg <- flag_outliers(df_smscg, Units_per_mL, cutoff = 5)
```

```{r}
# Biovolume per mL
df_smscg <- flag_outliers(df_smscg, Biovolume_per_mL)
```

# Save .Rdata
```{r}
save(df_smscg, file = 'admin/standardized_data/DWR-SMSCG.Rda')
```
