...
```{r}
df_aeu <- remove_nodata(df_aeu)
no_data <- attr(df_aeu, 'log')$nodata
```

## Check for outliers
```{r}
# Cells per mL
df_aeu <- flag_outliers(df_aeu, Cells_per_mL)
```

```{r}
# Units per mL
df_aeu <- flag_outliers(df_aeu, Units_per_mL)
```

```{r}
# Biovolume per mL
df_aeu <- flag_outliers(df_aeu, Biovolume_per_mL)
```
