...
rename_map <- c(
  ...
  'pole corer' = 'Bed Material -- pole corer',
  'unspecified grab sample' = 'Other',
  'wide mouth bottle' = 'Open-Mouth Bottle',
  'multiple sample methods' = 'Mulitple samplers used'
)
...
## Extract Taxon
```{r}
df_bgc <- extract_program_taxa(df_bgc)
```
...
## Add ID column
```{r}
df_bgc <- add_id_col(df_bgc, loc_col = 'Station')
```
...
```{r}
df_bgc <- combine_taxa(df_bgc, key_cols = c('Date', 'Time', 'Station', 'SampleDepth'))
combine_issues <- attr(df_bgc, 'log')$combined_conflicts
combine_checks <- attr(df_bgc, 'log')$combined_taxa
```

## Remove rows with no data
```{r}
df_bgc <- remove_nodata(df_bgc)
no_data <- attr(df_bgc, 'log')$nodata
```

## Check for outliers
```{r}
# Cells per mL
df_bgc <- flag_outliers(df_bgc, Cells_per_mL)
```

```{r}
# Biovolume per mL
df_bgc <- flag_outliers(df_bgc, Biovolume_per_mL, cutoff = 4)
```

## Save .Rdata
```{r}
save(df_bgc, file = 'admin/standardized_data/USGS-BGC.Rda')
```
