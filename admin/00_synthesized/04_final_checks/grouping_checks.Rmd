...
Density: Unit
Survey: DWR-EMP, USBR-DOP
Time: 2008-2024

```{r}
unique(df_pesp$Survey)
```


```{r}
df_std <- df_pesp %>%
  filter(Survey %in% c('DWR-EMP', 'USGS-BP'),
...
```{r}
sf_std <- convert_to_sf(df_std, sf_delta)
...
ggplot() +
  geom_sf(data = sf_delta) +
  geom_sf(data = sf_std.1, aes(shape = Region, fill = Survey), size = 3, alpha = 0.7) +
  scale_shape_manual(values = c(21, 23, 22, 24, 25, 25)) +
  guides(fill = guide_legend(override.aes = list(shape = 21, color = 'black')))
```

```{r}
com_std <- create_mcom(df_std.1,
                        group_var = c('Survey','Region','YearMonth'),
                        avg_var = 'Station',
                        taxa_var = 'AlgalGroup',
                        factor_var = c('Lab'),
...
```
```{r}
nmds_std.1 <- calc_nmds(df_std.1,
                        group_var = c('Survey','Region','YearMonth'),
                        avg_var = c('Station','Lab'),
                        taxa_var = 'AlgalGroup',
                        factor_var = c('Lab'),
                        measure_var = 'Units_per_mL',
                        distance = 'jaccard')
