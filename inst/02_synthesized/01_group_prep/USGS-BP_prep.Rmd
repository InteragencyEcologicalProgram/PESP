...
## Extract Taxon
```{r}
df_bp <- extract_program_taxons(df_bp)
```
...
```{r}
df_bp <- combine_taxons(df_bp, key_cols = c('Station','Date','SampleDepth','Lab','SampleMethod'), measurement_cols = c('Biovolume_per_mL','Cells_per_mL'))
...
## Save .Rdata
```{r}
save(df_bp, file = 'synthesized/02_group_data/USGS-BP.Rdata')
```
