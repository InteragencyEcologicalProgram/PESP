...
df_frp <- add_qc_col(df_frp, comment_col = Comments, key_cols = 'SampleID_frp', taxa_col = 'OriginalTaxon')
df_frp <- add_debris_col(df_frp, comment_col = Comments)
...
df_frp <- df_frp %>%
  ...
  select(-c(Kingdom,Phylum,Class,AlgalGroup,Genus,Species,CurrentTaxon,TrueTaxon,OrigTaxon,Taxonomy_EMP,Taxon)) %>%
