...
```{r}
df_data <- update_stations_bgc(df_data)
df_data <- df_data %>% mutate(Station = str_trim(Station))

df_data <- df_data %>%
  mutate(
    Station = case_when(Station == 'SFEI18' ~ 'PETER18',
              TRUE ~ Station)
  )
```
...
df_data <- df_data %>% select(-c(Latitude, Longitude))

df_data <- add_latlon(df_data, 'Groups/USGS-BGC/Metadata/Stations-BGC.csv')
```
